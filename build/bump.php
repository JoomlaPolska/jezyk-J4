...
$languagePackXmlFile = '/pkg_pl-PL.xml';
...
foreach ($iterator as $file)
{
	if ($file->isFile())
	{
		...
		if ($continue)
		{
			...
			// Check if need to change the copyright date.
			if (preg_match('#2008\s+-\s+[0-9]{4}\s+Polskie Centrum Joomla#', $fileContents) && !preg_match('#2008\s+-\s+' . $year . '\s+Polskie Centrum Joomla#', $fileContents))
			{
				$changeCopyrightDate = true;
				$fileContents = preg_replace('#2008\s+-\s+[0-9]{4}\s+Polskie Centrum Joomla#', '2008 - ' . $year . ' Polskie Centrum Joomla', $fileContents);
				$changedFilesCopyrightDate++;
			}
